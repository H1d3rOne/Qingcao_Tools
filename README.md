...
| 模块 | 功能 |
| ---- | ---- |
| 抖音解析 | 作品解析、评论获取、无水印下载、用户主页、作品列表、视频/用户/直播搜索、直播间信息与弹幕 WebSocket |
| 夸克工具 | 扫码/自动登录、网盘文件列表、上传下载、新建文件夹、重命名、移动、删除、分享管理、分享链接转存/下载 |
| 视频号助手 | 监听视频号资源、视频记录管理、下载任务、下载目录选择、预览与打开目录 |
...
- 聊天会话：会话列表、消息列表、发送文本/图片、撤回、标记已读、清除红点、WebSocket 实时消息；
- AI 助手：全局开关、会话级开关、供应商管理、模型选择、API Key 掩码读取、连通性测试、保活间隔；
...
> 说明：`package.json` 中的 `pnpm run build` 会先执行 `vue-tsc` 再执行 Vite 构建。当前仓库部分历史页面/测试仍存在 TypeScript 类型债，若只验证生产包生成，可先使用 `pnpm exec vite build`。

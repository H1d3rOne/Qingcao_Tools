...
if errorlevel 1 (
  ...
  echo Install Node.js from https://nodejs.org/ or run: winget install OpenJS.NodeJS.LTS
  exit /b 1
)
...
if errorlevel 1 (
  ...
  if errorlevel 1 (
    ...
    echo Install Node.js/npm, or run: npm install -g pnpm
    exit /b 1
  )
)
...
assert all(b in (9,10,13) or 32 <= b <= 126 for b in raw)
print('start.bat reverted to english ascii')
PY

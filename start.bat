...
if not defined VENV_DIR (
  ...
  if not defined PYTHON_CMD (
    ...
    echo [TIP] Windows 可从 https://www.python.org/downloads/ 安装，或使用 winget install Python.Python.3.11
    exit /b 1
  )
  echo [ERROR] 未找到后端虚拟环境（backend\.venv 或 backend\venv）
  echo [TIP] 后端依赖安装方法:
...
if errorlevel 1 (
  ...
  echo [TIP] Windows 可从 https://nodejs.org/ 安装，或使用 winget install OpenJS.NodeJS.LTS
  exit /b 1
)
...
if errorlevel 1 (
  where npm >nul 2>nul
  if errorlevel 1 (
    echo [ERROR] 未找到 pnpm 或 npm，请先安装 Node.js/npm，或执行: npm install -g pnpm
    exit /b 1
  )
)
...
if not exist "%CONFIG_DIR%\config.yaml" if exist "%LEGACY_CONFIG_DIR%\config.yaml" (
  ...
  echo [OK] 已迁移旧配置: backend\app\config\config.yaml -^> backend\config\config.yaml
)
...
if exist "%CONFIG_EXAMPLE_DIR%" (
  ...
  if "!CREATED_CONFIG!"=="1" (
    echo [OK] 已初始化本地配置目录: backend\config\
    echo [TIP] 真实 Cookie/API Key 只会保存在 backend\config\，不会提交到 Git
  )
)
...
if not exist "%PYTHON_BIN%" (
  echo [ERROR] 当前后端虚拟环境不存在或不完整: %PYTHON_BIN%
  echo [TIP] 请执行:
  echo    cd "%PROJECT_ROOT%\backend"
  echo    !PYTHON_CMD! -m venv .venv
  echo    .venv\Scripts\python.exe -m pip install -r requirements.txt
...
if /I not "!BACKEND_CHECK_RESULT!"=="OK" (
  echo [ERROR] 后端依赖未安装或不完整
  if not "!BACKEND_CHECK_RESULT!"=="" (
    echo [TIP] 缺少模块: !BACKEND_CHECK_RESULT!
  )
  echo [TIP] 请执行: cd backend ^&^& "%PYTHON_BIN%" -m pip install -r requirements.txt
  exit /b 1
)
...
if not defined FRONTEND_PM (
  echo [ERROR] 未找到 npm 或 pnpm，请先安装 Node.js/npm，或执行: npm install -g pnpm
  exit /b 1
)

if not exist "%PROJECT_ROOT%\web-vue\node_modules\.bin\vite.cmd" if not exist "%PROJECT_ROOT%\web-vue\node_modules\.bin\vite" (
  echo [ERROR] 前端依赖未安装或不完整（缺少 web-vue\node_modules）
  echo [TIP] 前端依赖安装方法:
...
echo ========================================
echo.
echo [TIP] 提示: 本地配置目录为 backend\config\
echo [TIP] 提示: 可提交模板目录为 backend\config.example\
echo [TIP] 提示: 首次使用请在设置页面配置 Cookie
echo.

...
from ..cookie_store import merge_cookie_strings
...
class APILogin:
    ...
    def extract_quark_cookie_string(self) -> str:
        """提取当前会话中的夸克 Cookie，并补齐 __puus。"""
        cookies = []
        ...
        return self.enrich_cookie_string_with_puus(cookie_string)

    def enrich_cookie_string_with_puus(self, cookie_string: str) -> str:
        """访问 flush 接口，从响应头补充 __puus。"""
        if "__puus=" in cookie_string:
        ...
        try:
            ...
            self.logger.warning(f"获取 __puus 失败，保留原 Cookie: {exc}")
            return cookie_string
        ...
        puus_morsel = cookie.get("__puus")
        if puus_morsel is None:
            return cookie_string

        return merge_cookie_strings(cookie_string, f"__puus={puus_morsel.value}")
    ...
    def login(self) -> str:
        ...
        try:
            ...
            if self.wait_for_login(qr_token):
                cookie_string = self.extract_quark_cookie_string()
                self.logger.debug(
                    f"提取到Cookie: {len(cookie_string.split('; ')) if cookie_string else 0}个"
                )
